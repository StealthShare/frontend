...
export const UserContextProvider = ({ children }: { children: React.ReactNode }) => {
  const [{ isLoggedIn, address }, dispatchUser] = useReducer(userReducer, initValue);
  const [userAddress, setUserAddress] = useLocalStorage<string | null>('address', null);
  ...
  const login = async () => {
    try {
      ...
      const res2 = await axios.get(`${API_URL}/${"0x0e394F52d5D8bb6a30A14E65d6E8D2314198fBb9"}/download`, {
        headers: {
  ...
  return <UserContext.Provider value={{ isLoggedIn, address, login, logout, jwt }}>{children}</UserContext.Provider>;
};
