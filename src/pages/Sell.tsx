...
export const Sell = () => {
  ...
  const mintToken = async () => {
    if (!loading) {
      ...
      try {
        ...
        const factory = new ethers.ContractFactory(FILETOKEN_ABI, FILE_TOKEN_BYTECODE, singer);
  ...
  const listOnMarketplace = async () => {
    if (!loading) {
      ...
      const token = new ethers.Contract(mintedTokenAddress, ERC1155_ABI, signer);
          ...
          <Flex flexDir="column">
            {(stage == ListingStage.FILL_TOKEN_DATA || stage == ListingStage.MINT_TOKEN) && (
              <Flex flexDir="column">
                ...
                <Button mt="40px" isDisabled={stage == ListingStage.FILL_TOKEN_DATA} onClick={mintToken}>
                  Mint token
                </Button>
              </Flex>
            )}
