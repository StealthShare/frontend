import { Box, Button, Flex, Grid, Input, Select, Spinner, Text } from '@chakra-ui/react';
  ...
  const listOnMarketplace = async () => {
    if (!loading) {
      try {
        ...
        const approval = await token.isApprovedForAll(await signer.getAddress(), MARKET_ADDRESS);
        alert(approval);
        if (approval == false) {
  ...
  return (
    <PageContainer>
      {stage == ListingStage.FINISHED ? (
        <Flex flexDir="column" justifyContent='center' align="center" minH="60vh">
          <Heading text="Your file was listed succesfully"/>
          <Box mt="10px" fontFamily="Inter">It will appear on marketplace in few minutes.</Box>
          <Button mt="20px" onClick={() => navigate("/marketplace")}>Go to marketplace</Button>
        </Flex>
      ) : (
              ...
              {(stage == ListingStage.FILL_TOKEN_DATA || stage == ListingStage.MINT_TOKEN) && (
                <Flex flexDir="column" gap="20px" fontSize="18px">
                          ...
                          {(TAGS as any)[fileType].map((tag: string) => {
                            return (
                              <Box
                                ...
                                bg={tags.includes(tag) ? 'brandPrimary' : 'none'}
                              >
                                {tag}
                              </Box>
                            );
                  ...
                  <Button
                    h="60px"
                    mt="20px"
                    isDisabled={stage == ListingStage.FILL_TOKEN_DATA || loading == true}
                    onClick={mintToken}
                  >
